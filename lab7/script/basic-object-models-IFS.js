...
function uvPyramida(radius, height, sides, noBottom) {
   radius = radius || 0.5;
   height = height || 2*radius;
   sides = sides || 11; // Zmiana na jedenastokąt
   var fractions = [ 0, 0.5, 0.75, 0.875, 0.9375 ];
   var vertexCount = fractions.length*(sides+1) + sides;
   if (!noBottom)
      vertexCount += sides + 2;
   var triangleCount = (fractions.length-1)*sides*2 + sides;
   if (!noBottom)
      triangleCount += sides;
   var vertices = new Float32Array(vertexCount*3);
   var normals = new Float32Array(vertexCount*3);
   ...
   for (j = 0; j < fractions.length-1; j++) {
      var row1 = j*(sides+1);
      var row2 = (j+1)*(sides+1);
      for (i = 0; i < sides; i++) {
          indices[k++] = row1 + i;
          indices[k++] = row2 + i + 1;
          indices[k++] = row2 + i;
          // Modyfikacja indeksów dla tworzenia trójkątów na ścianach bocznych
          indices[k++] = row1 + i;
          indices[k++] = row1 + i + 1;
          indices[k++] = row2 + i + 1;
      }
   }
   var start = kv/3 - (sides+1);
   for (i = 0; i < sides; i++) { // slices points at top, with different normals, texcoords
      u = (i+0.5)*du;
      var c = Math.cos(u);
      var s = Math.sin(u);
      vertices[kv] = 0;
      ...
      vertices[kv] = height/2;
      normals[kv++] = n2;
      texCoords[kt++] = (i+0.5)/sides;
      texCoords[kt++] = 1;
   }
   for (i = 0; i < sides; i++) {
      indices[k++] = start+i;
      indices[k++] = start+i+1;
      indices[k++] = start+(sides+1)+i;
   }
   if (!noBottom) {
      var startIndex = kv/3;
      vertices[kv] = 0;
